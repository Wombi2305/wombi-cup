    ...
    const fetchMembers = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('team_members')
          .select('id, user_id, role, profiles(id, ea_ingame_name, discord_name)')
                        ...
                        <div key={member.id} className="bg-black/40 border border-white/10 rounded-xl p-3 sm:p-4 flex flex-col relative overflow-hidden group">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col overflow-hidden pr-12">
                              ...
                              {/* --- NEU: HIER WIRD DER DISCORD NAME ANGEZEIGT --- */}
                              {member.profiles?.discord_name && (
                                <span className="text-[#5865F2] text-[10px] font-semibold truncate mt-0.5 flex items-center gap-1">
                                  <svg width="10" height="10" viewBox="0 0 127.14 96.36" fill="currentColor">
