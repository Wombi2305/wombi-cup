...
export default function ProfilPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (teamsData && teamsData.length > 0) {
        ...
        const activeTeam = teamsData.find(t => t.is_active) || teamsData[0];
        handleSelectTeam(activeTeam, false);
      } else {
        setIsCreating(true);
      }
  ...
  const confirmDeleteTeam = async () => {
    ...
    try {
      ...
      if (remainingTeams.length > 0) {
        handleSelectTeam(remainingTeams[0], false);
      } else {
        handleSelectTeam(null, true);
      }
  ...
  const teamStats = calculateTeamLevel(currentTeam);
            ...
            {hasFormAccess ? (
              <div className="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8 shadow-xl h-full flex flex-col">
                  ...
                  <div className="mb-8 bg-black/10 rounded-3xl p-6 relative overflow-hidden">
                    <div className="absolute top-4 right-4 z-10">
                      {currentTeam.is_active ? (
                        <span className="bg-green-500/20 text-green-400 border border-green-500/30 px-3 py-1 rounded-full text-xs font-bold uppercase flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse"></span> Aktiv
                        </span>
                      ) : (
                        <button onClick={handleMakeActive} disabled={saving} className="bg-white/10 hover:bg-white/20 text-white border border-white/20 px-3 py-1 rounded-full text-xs font-bold uppercase transition">
                          Als Aktiv setzen
                        </button>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 mb-5 mt-4">
                      <img 
                        ...
                        className="w-28 h-28 sm:w-36 sm:h-36 object-contain mix-blend-screen transition-transform hover:scale-105"
                      />
                    ...
                    <div className="grid grid-cols-5 gap-2 sm:gap-3 text-center text-xs text-gray-400 mt-6">
                      ...
                      <div className="bg-white/5 border border-white/5 rounded-xl p-2 flex flex-col justify-center">
                        ...
                        <span className="truncate">Tore</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <form onSubmit={handleSaveTeam} className="flex flex-col gap-5 mt-auto">
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-gray-500 font-bold mb-2 ml-1">
                      {isCreating ? "Teamname" : "Teamname (kann nicht geändert werden)"}
                    </label>
                    <input 
                      type="text" 
                      value={teamname} 
                      ...
                      disabled={!isCreating}
                      className={`w-full bg-black/40 border border-white/10 rounded-xl p-4 text-white placeholder-gray-600 focus:outline-none focus:border-yellow-500 transition ${!isCreating ? 'opacity-50 cursor-not-allowed' : ''}`} 
                    />
                  </div>
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-gray-500 font-bold mb-2 ml-1">
                      {isCreating ? "Captain (Ingame Name)" : "Captain (kann nicht geändert werden)"}
                    </label>
                    <input 
                      type="text" 
                      value={captain} 
                  ...
                  <div className="mt-2 flex flex-col gap-3">
                    {isCreating ? (
                      <button 
                        type="submit" 
                        disabled={saving} 
                        className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-bold py-4 rounded-xl transition shadow-lg shadow-yellow-500/20 disabled:opacity-50"
                      >
                        {saving ? "Speichern..." : "Team erstellen"}
                      </button>
                    ) : (
                      <button 
                        type="button" 
                        ...
                        disabled={saving}
                        className="w-full bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 font-bold py-4 px-6 rounded-xl transition disabled:opacity-50"
                      >
                        Team löschen
                      </button>
                    )}
              ...
              <div className="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-10 shadow-xl flex flex-col items-center justify-center text-center h-full">
                ...
                <a href="https://discord.gg/2QsPJ6r5" target="_blank" rel="noreferrer" className="bg-[#5865F2] hover:bg-[#4752C4] text-white font-bold py-3.5 px-8 rounded-xl transition shadow-lg shadow-[#5865F2]/20 flex items-center justify-center gap-3 w-full sm:w-auto">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-[#111] border border-red-500/30 rounded-2xl p-6 md:p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-gray-400 mb-6 text-sm leading-relaxed">
              Diese Aktion entfernt das Team aus deinem Profil. Um Missbrauch zu verhindern, gib bitte den exakten Namen <strong className="text-red-400">{currentTeam.teamname}</strong> ein.
            </p>
            ...
            <input 
              ...
              placeholder="Teamname bestätigen"
              className="w-full bg-black/40 border border-red-500/20 rounded-xl p-4 text-white focus:outline-none focus:border-red-500 transition mb-6 placeholder-gray-600"
            />
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                disabled={deleteConfirmName !== currentTeam.teamname || saving}
                className="flex-1 bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
