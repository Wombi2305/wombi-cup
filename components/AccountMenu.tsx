...
export default function AccountMenu() {
  // 🔥 User blitzschnell aus dem globalen State holen
  const { user } = useAuth();
  ...
  // Discord Nickname UND Rollen laden, sobald ein User da ist
  useEffect(() => {
  ...
  const logout = async () => {
    ...
    setUserRoles([]);
    setOpen(false);
    // Reload ist in Ordnung, um wirklich alle States hart zu clearen
    window.location.reload(); 
  };

  // Wenn kein User da ist, wird das Menü gar nicht erst gerendert
  if (!user) return null;
